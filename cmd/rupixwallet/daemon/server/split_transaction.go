...
func (s *server) mergeTransaction(
	...
	utxos := make([]*librupixwallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &librupixwallet.UTXO{
	...
	return librupixwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by rupixwallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*librupixwallet.UTXO, 0, endIndex-startIndex)
	totalrupia := uint64(0)
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return librupixwallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*librupixwallet.UTXO, requiredAmount uint64, feeRate float64) (
	additionalUTXOs []*librupixwallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &librupixwallet.UTXO{
