...
func (c *kaspawalletdClient) Forge(ctx context.Context, in *ForgeRequest, opts ...grpc.CallOption) (*ForgeResponse, error) {
	out := new(ForgeResponse)
	err := c.cc.Invoke(ctx, "/kaspawalletd.kaspawalletd/Forge", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _Kaspawalletd_Forge_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ForgeRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(KaspawalletdServer).Forge(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/kaspawalletd.kaspawalletd/Forge",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(KaspawalletdServer).Forge(ctx, req.(*ForgeRequest))
	}
	return interceptor(ctx, in, info, handler)
}
