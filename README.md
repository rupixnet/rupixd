...
- ✅ **Third external node** (16-Sep-2026): a third participant synced their node from scratch with the RupixHeavyHash testnet (v0.6.0) and received RUPIX. The public network runs on 3 nodes: the seed server and two external ones.
...
- ⚠️ **Consensus bug found by the new end-to-end King test** (21-Sep-2026): the miner double-counted forges in the block it was building (a 13-Sep patch), while the validator counts what the block *accepts*. Any production-mined block containing a forge would have been rejected — the first real King on mainnet included. Fix ready on branch `king-e2e` (test fails if H-10 is reverted; full suite green). Ships in the next consensus relaunch (v0.6.0) together with the keccak domain and integer rank. Found on testnet, not mainnet.
...
Every block you find pays you Gold. With that Gold you can forge gems (see [GUIA-FORJA.md](./GUIA-FORJA.md)). Mining on testnet has no monetary value: it helps the network grow its hashrate on the way to mainnet. The more honest, distributed hashrate, the more secure the network — mining is the most direct way to contribute to Rupix.
...
- ✅ **Temporary checkpoints — DONE (v0.6.0)** — with expiry inside consensus, tested on devnet, public policy in [CHECKPOINTS.md](./CHECKPOINTS.md). The first real one is yet to be published.
