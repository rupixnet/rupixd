...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of rupixd instances
	Rupixd1, Rupixd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := Rupixd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In Rupixd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, Rupixd2)
	}

	// Connect the two Rupixds. This should trigger sync
	// between the two nodes
	connect(t, Rupixd1, Rupixd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := Rupixd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
