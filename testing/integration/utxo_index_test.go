...
func TestUTXOIndex(t *testing.T) {
	// Setup a single rupixd instance
	...
	err := rupixd.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := rupixd.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedrupia := uint64(blockAmountToMine * constants.RupiaPerRupix * 500)
	getBlockCountResponse, err := rupixd.rpcClient.GetBlockCount()
	if err != nil {
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = rupixd.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := rupixd.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
