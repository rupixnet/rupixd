...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "Rupixdsvc",
	DisplayName: "rupixd Service",
	Description: "Downloads and stays synchronized with the rupix blockDAG and " +
		"provides DAG services to applications.",
}

type RupixdApp struct {
	cfg *config.Config
}

// StartApp starts the rupixd app, and blocks until it finishes running
func StartApp() error {
...
func (app *RupixdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start rupixd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down rupixd...")
		...
		log.Infof("rupixd shutdown complete")
	}()
