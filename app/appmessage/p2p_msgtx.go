...
// Outpoint defines a rupix data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new rupix transaction outpoint point with the
...
// TxIn defines a rupix transaction input.
type TxIn struct {
...
// NewTxIn returns a new rupix transaction input with the provided
...
// TxOut defines a rupix transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new rupix transaction output with the provided
...
// MsgTx implements the Message interface and represents a rupix tx message.
