...
func (mp *mempool) checkTransactionStandardInIsolation(transaction *externalapi.DomainTransaction) error {
	...
	// However, in a later version of rupix the consensus-valid transaction version range might diverge from the
...
func (mp *mempool) IsTransactionOutputDust(output *externalapi.DomainTransactionOutput) bool {
	...
	// mp.config.MinimumRelayTransactionFee is in rupia/KB, so multiply
	// by 1000 to convert to bytes.
	//
	// Using the typical values for a pay-to-pubkey transaction from
	// the breakdown above and the default minimum free transaction relay
	// fee of 1000, this equates to values less than 546 rupia being
	// considered dust.
	//
...
func (mp *mempool) minimumRequiredTransactionRelayFee(mass uint64) uint64 {
	...
	// rupia/kg so multiply by mass (which is in grams) and divide by 1000 to get minimum rupias.
	...
	if minimumFee > constants.MaxRupia {
		minimumFee = constants.MaxRupia
	}
