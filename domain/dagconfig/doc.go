...
In addition to the main rupix network, which is intended for the transfer
...
variable (either directly, or hidden in a library call).
	...
	var testnet = flag.Bool("testnet", false, "operate on the testnet rupix network")
...
If an application does not use one of the standard rupix networks, a new
