...
func (v *transactionValidator) checkLevelRules(tx *externalapi.DomainTransaction, povDaaScore uint64, isCoinbase bool) error {
	...
	// Burn por transaccion (Rupix): toda tx no-coinbase debe quemar al menos
	// BurnBase + BurnPerByte * bytes en un output OpReturn de Gold. "Al menos":
	// un ascenso Gold->Diamante ya quema 10 RUPIX y cubre esta regla de sobra —
	// un solo output de quema sirve para ambas leyes.
	if !isCoinbase {
		txBytes := txmass.TransactionSerializedSize(tx)
		requerido := v.burnBase + v.burnPerByte*txBytes
		if burnedGold < requerido {
			return errors.Wrapf(ruleerrors.ErrInsufficientBurn,
				"la tx quema %d rupias en OpReturn; el burn por transaccion exige al menos %d (%d bytes)",
				burnedGold, requerido, txBytes)
		}
	}
