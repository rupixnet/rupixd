...
func (v *transactionValidator) validateLevelBurn(tx *externalapi.DomainTransaction, povBlockHash *externalapi.DomainHash, stagingArea *model.StagingArea) error {
	...
	case constants.LevelDiamante:
		if povDAAScore < v.levelDiamanteUnlockScore {
			return errors.Wrapf(ruleerrors.ErrInvalidLevelBurn, "nivel Diamante no desbloqueado aun (score actual: %d, requerido: %d)", povDAAScore, v.levelDiamanteUnlockScore)
		}
	case constants.LevelPlatino:
		if povDAAScore < v.levelPlatinoUnlockScore {
			return errors.Wrapf(ruleerrors.ErrInvalidLevelBurn, "nivel Platino no desbloqueado aun (score actual: %d, requerido: %d)", povDAAScore, v.levelPlatinoUnlockScore)
		}
	case constants.LevelRodio:
		if povDAAScore < v.levelRodioUnlockScore {
			return errors.Wrapf(ruleerrors.ErrInvalidLevelBurn, "nivel Rodio no desbloqueado aun (score actual: %d, requerido: %d)", povDAAScore, v.levelRodioUnlockScore)
		}
	case constants.LevelKings:
		if povDAAScore < v.levelKingsUnlockScore {
			return errors.Wrapf(ruleerrors.ErrInvalidLevelBurn, "Kings no desbloqueado aun (score actual: %d, requerido: %d)", povDAAScore, v.levelKingsUnlockScore)
		}
	default:
