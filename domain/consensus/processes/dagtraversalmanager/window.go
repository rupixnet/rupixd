...
// If the number of blocks in the past of startingNode is less than windowSize,
// the window will be smaller than windowSize.
func (dtm *dagTraversalManager) BlockWindow(stagingArea *model.StagingArea, blockHash *externalapi.DomainHash, windowSize int) ([]*externalapi.DomainHash, error) {
	windowHeap, err := dtm.blockWindowHeap(stagingArea, blockHash, windowSize)
	if err != nil {
		return nil, err
	}
...
func (dtm *dagTraversalManager) calculateBlockWindowHeap(stagingArea *model.StagingArea,
	...
	// Walk down the chain until you finish or find a trusted block and then complete the rest
	// of the window with the trusted window.
	for {
		// FIX: Guard against nil SelectedParent (genesis block has no selected parent)
...
func (dtm *dagTraversalManager) tryPushMergeSet(windowHeap *sizedUpBlockHeap, currentGHOSTDAGData, selectedParentGHOSTDAGData *externalapi.BlockGHOSTDAGData) (bool, error) {
	...
	// FIX: Guard against empty MergeSetBlues — genesis block has no blues, [1:] on empty slice = panic
	rawMergeSetBlues := currentGHOSTDAGData.MergeSetBlues()
	if len(rawMergeSetBlues) > 0 {
		// Remove the SP from the blue merge set because we already added it.
		mergeSetBlues := rawMergeSetBlues[1:]
