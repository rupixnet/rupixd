...
func (bp *blockProcessor) validateAndInsertBlockWithTrustedData(stagingArea *model.StagingArea,
    ...
    if err != nil {
        fmt.Printf("FAIL ghostdagDataWithoutPrunedBlocks: %+v\n", err)
    ...
    result, status, err := bp.validateAndInsertBlock(stagingArea, block.Block, false, validateUTXO, true)
    if err != nil {
        fmt.Printf("FAIL validateAndInsertBlock: %+v\n", err)
    }
...
func (bp *blockProcessor) ghostdagDataWithoutPrunedBlocks(stagingArea *model.StagingArea,
    ...
    for _, blockHash := range data.MergeSetBlues() {
        ...
        if isPruned {
            if data.SelectedParent() != nil && data.SelectedParent().Equal(blockHash) {
