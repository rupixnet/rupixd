package burnmanager

import (
	"github.com/rupixnet/rupixd/domain/consensus/datastructures/burnstatsstore"
	"github.com/rupixnet/rupixd/domain/consensus/model"
	"github.com/rupixnet/rupixd/domain/consensus/model/externalapi"
	"github.com/rupixnet/rupixd/domain/consensus/ruleerrors"
	"github.com/pkg/errors"
)

const (
	TokenLevelGold     = uint8(1)
	TokenLevelDiamond  = uint8(2)
	TokenLevelPlatinum = uint8(3)
	TokenLevelRhodium  = uint8(4)
	TokenLevelKings    = uint8(5)

	BurnRatio      = uint64(10)
	BurnPercent    = uint64(98)
	CausasPercent  = uint64(1)
	MejorasPercent = uint64(1)
	MaxLevel       = TokenLevelKings

	BurnAddress         = "rupix:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq"
	FondoCausasAddress  = "rupix:qr7yantnyfhhhw7d8duhdax22q3cmwzhnee9lw9q0xmdvrnla62jkztf63peq"
	FondoMejorasAddress = "rupix:qpna2974ylzpcsdnavsk5adust7rpngkepkyl6zzdjlh8z5vemqxjfxce3und"
)

var MaxSupplyByLevel = map[uint8]uint64{
	TokenLevelGold:     42_000_000,
	TokenLevelDiamond:  2_100_000,
	TokenLevelPlatinum: 210_000,
	TokenLevelRhodium:  21_000,
	TokenLevelKings:    2_100,
}

var LevelNames = map[uint8]string{
	TokenLevelGold:     "Rupix Gold L1",
	TokenLevelDiamond:  "Rupix Diamante L2",
	TokenLevelPlatinum: "Rupix Platino L3",
	TokenLevelRhodium:  "Rupix Rodio L4",
	TokenLevelKings:    "Kings Rupix L5",
}

type BurnResult struct {
	BurnTx           *externalapi.DomainTransaction
	ToLevel          uint8
	MintedAmount     uint64
	RecipientAddress string
	BurnAmount       uint64
	CausasAmount     uint64
	MejorasAmount    uint64
	CausasAddress    string
	MejorasAddress   string
}

type BurnStats struct {
	TotalMined       map[uint8]uint64
	TotalBurned      map[uint8]uint64
	TotalMinted      map[uint8]uint64
	CurrentSupply    map[uint8]uint64
	MaxSupply        map[uint8]uint64
	TotalRupixBurned uint64
	TotalRupixMined  uint64
	CirculatingL1    uint64
	PercentageBurned float64
}

type Manager interface {
	ValidateBurnTransaction(tx *externalapi.DomainTransaction) error
	ProcessBurnTransaction(tx *externalapi.DomainTransaction) (*BurnResult, error)
	GetBurnStats() (*BurnStats, error)
	IsBurnAddress(address string) bool
}

type burnManager struct {
	stats     *BurnStats
	store     *burnstatsstore.BurnStatsStore
	dbContext model.DBManager
}

func New(store *burnstatsstore.BurnStatsStore, dbContext model.DBManager) Manager {
	bm := &burnManager{
		store:     store,
		dbContext: dbContext,
		stats: &BurnStats{
			TotalMined:    make(map[uint8]uint64),
			TotalBurned:   make(map[uint8]uint64),
			TotalMinted:   make(map[uint8]uint64),
			CurrentSupply: make(map[uint8]uint64),
			MaxSupply:     MaxSupplyByLevel,
		},
	}
	// Cargar stats persistidos al iniciar
	if store != nil && dbContext != nil {
		if persisted, err := store.Load(dbContext); err == nil {
			bm.stats.TotalBurned = persisted.TotalBurned
			bm.stats.TotalMinted = persisted.TotalMinted
			bm.stats.CurrentSupply = persisted.CurrentSupply
			bm.stats.TotalRupixBurned = persisted.TotalRupixBurned
			bm.stats.TotalRupixMined = persisted.TotalRupixMined
			bm.stats.CirculatingL1 = persisted.CirculatingL1
			bm.stats.PercentageBurned = persisted.PercentageBurned
		}
	}
	return bm
}

func (bm *burnManager) IsBurnAddress(address string) bool {
	return address == BurnAddress
}

func (bm *burnManager) ValidateBurnTransaction(tx *externalapi.DomainTransaction) error {
	if len(tx.Inputs) == 0 {
		return errors.Wrapf(ruleerrors.ErrNoTxInputs, "burn transaction cannot be coinbase")
	}
	if len(tx.Outputs) == 0 {
		return errors.Wrapf(ruleerrors.ErrNoTxInputs, "burn transaction has no outputs")
	}
	if len(tx.Payload) < 2 {
		return errors.Wrapf(ruleerrors.ErrInvalidPayload, "burn transaction payload too small")
	}
	sourceLevel := tx.Payload[0]
	targetLevel := tx.Payload[1]
	if targetLevel != sourceLevel+1 {
		return errors.Wrapf(ruleerrors.ErrInvalidPayload, "invalid level transition: %d->%d", sourceLevel, targetLevel)
	}
	if targetLevel > TokenLevelKings {
		return errors.Wrapf(ruleerrors.ErrInvalidPayload, "invalid target level %d: max is %d", targetLevel, TokenLevelKings)
	}
	if sourceLevel < TokenLevelGold {
		return errors.Wrapf(ruleerrors.ErrInvalidPayload, "invalid source level %d: min is %d", sourceLevel, TokenLevelGold)
	}
	var totalBurned uint64
	for _, out := range tx.Outputs {
		totalBurned += out.Value
	}
	if totalBurned < BurnRatio {
		return errors.Wrapf(ruleerrors.ErrInvalidPayload, "burn amount %d less than required %d", totalBurned, BurnRatio)
	}
	return nil
}

func (bm *burnManager) ProcessBurnTransaction(tx *externalapi.DomainTransaction) (*BurnResult, error) {
	if err := bm.ValidateBurnTransaction(tx); err != nil {
		return nil, err
	}
	fromLevel := tx.Payload[0]
	toLevel := tx.Payload[1]

	currentMinted := bm.stats.TotalMinted[toLevel]
	maxSupply := MaxSupplyByLevel[toLevel]
	if currentMinted >= maxSupply {
		return nil, errors.Errorf("maximum supply for level %d (%s) reached: %d/%d",
			toLevel, LevelNames[toLevel], currentMinted, maxSupply)
	}

	var totalBurned uint64
	for _, out := range tx.Outputs {
		totalBurned += out.Value
	}

	burnAmount    := totalBurned * BurnPercent / 100
	causasAmount  := totalBurned * CausasPercent / 100
	mejorasAmount := totalBurned * MejorasPercent / 100

	bm.stats.TotalBurned[fromLevel] += BurnRatio
	bm.stats.TotalMinted[toLevel]++
	bm.stats.CurrentSupply[fromLevel] -= BurnRatio
	bm.stats.CurrentSupply[toLevel]++

	if fromLevel == TokenLevelGold {
		bm.stats.TotalRupixBurned += BurnRatio
		bm.stats.CirculatingL1 -= BurnRatio
		if bm.stats.TotalRupixMined > 0 {
			bm.stats.PercentageBurned = float64(bm.stats.TotalRupixBurned) / float64(bm.stats.TotalRupixMined) * 100
		}
	}

	// Persistir stats en DB
	if bm.store != nil && bm.dbContext != nil {
		_ = bm.store.Save(bm.dbContext, &burnstatsstore.PersistedBurnStats{
			TotalBurned:      bm.stats.TotalBurned,
			TotalMinted:      bm.stats.TotalMinted,
			CurrentSupply:    bm.stats.CurrentSupply,
			TotalRupixBurned: bm.stats.TotalRupixBurned,
			TotalRupixMined:  bm.stats.TotalRupixMined,
			CirculatingL1:    bm.stats.CirculatingL1,
			PercentageBurned: bm.stats.PercentageBurned,
		})
	}

	return &BurnResult{
		BurnTx:         tx,
		ToLevel:        toLevel,
		MintedAmount:   1,
		BurnAmount:     burnAmount,
		CausasAmount:   causasAmount,
		MejorasAmount:  mejorasAmount,
		CausasAddress:  FondoCausasAddress,
		MejorasAddress: FondoMejorasAddress,
	}, nil
}

func (bm *burnManager) GetBurnStats() (*BurnStats, error) {
	return bm.stats, nil
}

func LevelName(level uint8) string {
	if name, ok := LevelNames[level]; ok {
		return name
	}
	return "Nivel Invalido"
}