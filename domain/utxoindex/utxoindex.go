...
func (ui *UTXOIndex) Reset() error {
	...
	err = ui.store.initializeCirculatingrupiaSupply() //At this point the database is empty, so the sole purpose of this call is to initialize the circulating supply key
	if err != nil {
		return err
	}
...
func (ui *UTXOIndex) GetCirculatingrupiaSupply() (uint64, error) {
	...
	return ui.store.getCirculatingrupiaSupply()
}
