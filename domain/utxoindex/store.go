...
func (uis *utxoIndexStore) commit() error {
	...
	for scriptPublicKeyString, toRemoveUTXOOutpointEntryPairs := range uis.toRemove {
		...
		for outpointToRemove, utxoEntryToRemove := range toRemoveUTXOOutpointEntryPairs {
			...
			toRemoverupiaSupply = toRemoverupiaSupply + utxoEntryToRemove.Amount()
		}
	}

	toAddrupiaSupply := uint64(0)
	...
	for scriptPublicKeyString, toAddUTXOOutpointEntryPairs := range uis.toAdd {
		...
		for outpointToAdd, utxoEntryToAdd := range toAddUTXOOutpointEntryPairs {
			...
			toAddrupiaSupply = toAddrupiaSupply + utxoEntryToAdd.Amount()
		}
	}
	...
	err = uis.updateCirculatingrupiaSupply(dbTransaction, toAddrupiaSupply, toRemoverupiaSupply)
	if err != nil {
		return err
	}
...
func (uis *utxoIndexStore) addAndCommitOutpointsWithoutTransaction(utxoPairs []*externalapi.OutpointAndUTXOEntryPair) error {
	...
	for _, pair := range utxoPairs {
		...
		toAddrupiaSupply = toAddrupiaSupply + pair.UTXOEntry.Amount()
	}

	err := uis.updateCirculatingrupiaSupplyWithoutTransaction(toAddrupiaSupply, uint64(0))
	if err != nil {
		return err
	}
...
func (uis *utxoIndexStore) initializeCirculatingrupiaSupply() error {
...
func (uis *utxoIndexStore) updateCirculatingrupiaSupply(dbTransaction database.Transaction, toAddrupiaSupply uint64, toRemoverupiaSupply uint64) error {
	if toAddrupiaSupply != toRemoverupiaSupply {
		...
		err = dbTransaction.Put(
			circulatingSupplyKey,
			binaryserialization.SerializeUint64(circulatingSupply+toAddrupiaSupply-toRemoverupiaSupply),
		)
		if err != nil {
			return err
		}
...
func (uis *utxoIndexStore) updateCirculatingrupiaSupplyWithoutTransaction(toAddrupiaSupply uint64, toRemoverupiaSupply uint64) error {
	if toAddrupiaSupply != toRemoverupiaSupply {
...
func (uis *utxoIndexStore) getCirculatingrupiaSupply() (uint64, error) {
	if uis.isAnythingStaged() {
