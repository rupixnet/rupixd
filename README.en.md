...
- ✅ **Third external node** (16-Sep-2026): a third participant synced their node from scratch with the RupixHeavyHash testnet (v0.5.1) and received RUPIX. The public network runs on 3 nodes: the seed server and two external ones.
...
- ✅ **Mining accessible to everyone — DONE (v0.5.1)** — Rupix migrated from Kaspa's inherited algorithm to RupixHeavyHash, its own algorithm. Kaspa ASICs can no longer mine Rupix; it is mined from an ordinary computer (GPU/CPU). Rupix is for everyone.
...
- ✅ **Temporary checkpoints — DONE (v0.5.1)** — with expiry inside consensus, tested on devnet, public policy in [CHECKPOINTS.md](./CHECKPOINTS.md). The first real one is yet to be published.
